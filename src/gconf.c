      ...
      if(!strcmp(key, "reload"))
      {
        ...
        if(!strcmp(key, "zoom-full"))
        {
          ...
          if(!strcmp(key, "writesave"))
          {
            ...
            if(!strcmp(key, "add-signal-fac-offset"))
            {
              if((str)&&(str[0]))
              {
                str2 = parse_rpc_id(str, &this_wave_rpc_id);
                if((this_wave_rpc_id == wave_rpc_id) && str2)
                {
                  if(!in_main_iteration())
                  {
                    ...
                    int _i = atoi(str2);
                    if (_i < GLOBALS->numfacs && _i >= 0) {
                      set_window_busy(NULL);  
                      AddNodeUnroll(GLOBALS->facs[_i]->n, NULL);
                  ...
                  g_settings_set(settings, "add-signal-fac-offset", "s", "");
                }
              }
            }

          else
            if(!strcmp(key, "dump-signal-fac-offset"))
            {
              if((str)&&(str[0]))
              {
                str2 = parse_rpc_id(str, &this_wave_rpc_id);
                if((this_wave_rpc_id == wave_rpc_id) && str2)
                {
                  ...
                  {
                    FILE* dumpfile;
                    dumpfile = fopen(str2,"w+");
                    int _i;
                    if (dumpfile){
                      for (_i=0; _i < GLOBALS->numfacs; _i++){
                        fprintf(dumpfile, "%s %d\n",GLOBALS->facs[_i]->name, _i);
                      }

                      fclose(dumpfile);
                    } else {
                      printf("%s was the string we tried to print to\n", str2);

                    }
                  }
                  g_settings_set(settings, "dump-signal-fac-offset", "s", "");
                }
              }
            }
              ...
              if((str)&&(str[0]))
              {
                ...
                if((this_wave_rpc_id == wave_rpc_id) && str2)
                {
                  if(!in_main_iteration())
                  {
                    ...
                    set_window_busy(NULL);
                    for (_i=0; _i < GLOBALS->numfacs; _i++){
                      if (strstr(GLOBALS->facs[_i]->name, str2)){
                        AddNodeUnroll(GLOBALS->facs[_i]->n, NULL);
                      }
                    }
                    GLOBALS->traces.scroll_top = GLOBALS->traces.scroll_bottom = GLOBALS->traces.last;
                  ...
                  g_settings_set(settings, "add-signal-name", "s", "");
                }
              }
            }
